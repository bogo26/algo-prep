...
var drawWire = function (wire) {
    ...
    for (let i = 0; i < wire.length; i++) {
        instruction = wire[i][0];
        distance = parseInt(wire[i].slice(1, wire[i].length));

        console.log('instruction>> ', instruction);

        switch (instruction) {
            case "R":
                for (let k = pointerX + 1; k <= pointerX + distance; k++) {
                    if (board[k]) {
                        if (board[k][pointerY] === 1) {
                            if (minVal > Math.abs(k) + Math.abs(pointerY)) {
                                minVal = Math.abs(k) + Math.abs(pointerY)
                            }
                            // console.log(k, pointerY);
                            board[k][pointerY] = 2;
                        } else {
                            board[k][pointerY] = 1;
                        }
            ...
            case "U":
                for (let k = pointerY + 1; k <= pointerY + distance; k++) {
                    if (board[pointerX]) {
                        if (board[pointerX][k] === 1) {
                            if (minVal > Math.abs(pointerX) + Math.abs(k)) {
                                minVal = Math.abs(pointerX) + Math.abs(k)
                            }
                            console.log(pointerX, k);
                            board[pointerX][k] = 2;
                        } else {
                            board[pointerX][k] = 1;
                        }
            ...
            case "L":
                for (let k = pointerX - distance; k > pointerX; k++) {
                    if (board[k]) {
                        if (board[k][pointerY] === 1) {
                            if (minVal > Math.abs(k) + Math.abs(pointerY)) {
                                minVal = Math.abs(k) + Math.abs(pointerY)
            ...
            case "D":
                for (let k = pointerY - distance; k < pointerY; k++) {
                    if (board[pointerX]) {
                        if (board[pointerX][k] === 1) {
                            if (minVal > Math.abs(pointerX) + Math.abs(k)) {
                                minVal = Math.abs(pointerX) + Math.abs(k)
        ...
        console.log('pinter: ', pointerX, pointerY);
    }
}
